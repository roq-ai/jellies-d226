...
import { deleteCartItemById } from 'apiSdk/cart-items';
...
function ProductViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface>(
    ...
    () =>
      getProductById(id, {
        relations: ['company', 'cart_item', 'review'],
      }),
  );

  const cart_itemHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('cart_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/cart-items/create?product_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.cart_item?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/cart-items/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('cart_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/cart-items/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('cart_item', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    cart_itemHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/reviews/create?product_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
